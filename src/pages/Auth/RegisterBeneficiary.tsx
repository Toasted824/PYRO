...
export function RegisterBeneficiary() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    try {
      const u = register({ ...form, role: 'beneficiary', name: form.name.trim(), email: form.email.trim() })
      ...
      <div className="max-w-[560px] mx-auto px-4 sm:px-6 pt-8 pb-12">
        <div className="bg-white rounded-[24px] border border-stone-200 shadow-sm p-6 sm:p-8">
          ...
          <form onSubmit={submit} className="mt-6 space-y-4">
            ...
            <button type="submit" className="w-full bg-sky-500 hover:bg-sky-600 text-white font-extrabold py-3.5 rounded-full shadow transition">Create beneficiary account →</button>
          </form>
