            ...
            {form.password.length > 0 && (
              <div className="flex items-center gap-2 mt-1">
                <div className="flex-1 h-1.5 rounded-full bg-stone-100 overflow-hidden">
                  <div className={`h-full rounded-full transition-all ${form.password.length >= 12 ? 'bg-sky-500 w-full' : form.password.length >= 8 ? 'bg-amber-400 w-2/3' : form.password.length >= 6 ? 'bg-amber-300 w-1/3' : 'bg-red-400 w-[15%]'}`} />
                </div>
  ...
  return (
    <div>
      ...
      <div className="relative">
        <input id={id} type={isPassword && show ? 'text' : type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="mt-1 w-full rounded-xl border border-stone-200 bg-sky-50/20 px-4 py-3 pr-12 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500" />
        {isPassword && (
